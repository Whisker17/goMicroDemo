...
func (s *Say) Stream(ctx context.Context, req *model.SRequest, stream rpcapi.Say_StreamStream) error {
	...
	for i := 0; i < int(req.Count); i++ {
		rsp := &model.SResponse{}
		for j := util.Random(3, 5); j < 10; j++ {
			rsp.Value = append(rsp.Value, util.RandomStr(util.Random(3, 10)))
		}
...
func (s *Say) BidirectionalStream(ctx context.Context, stream rpcapi.Say_BidirectionalStreamStream) error {
	for {
		...
		if err := stream.Send(&model.SResponse{Value: []string{util.RandomStr(util.Random(3, 6))}}); err != nil {
			return err
		}
	}
